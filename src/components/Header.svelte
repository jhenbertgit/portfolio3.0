<script lang="ts">
	...
	import { drawerOpenDark, drawerOpenLight, drawerCloseDark, drawerCloseLight } from '$lib/img';
...
{#if mounted}
	<header
		id="header"
		class="{!$isDrawerOpen && $isScrolled ? 'scrolled-state' : ''}
	...
	>
		<div class="absolute top-1/2 -translate-y-1/2 left-0 ml-5">
			<a href="/" class="headerBrandLink flex hover:scale-105">
				<img width={15} height={20} alt="" src={$isDarkMode ? brand.dark : brand.light} />
		...
		<nav class="absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2">
			{#if !$isDrawerOpen}
				<div class="hidden md:flex gap-3">
					{#each [{ text: 'Home', url: '/' }, { text: 'About', url: '/about' }, { text: 'Projects', url: '/projects' }, { text: 'Blog', url: '/posts' }, { text: 'Contact', url: '/contact' }] as route}
						<a
							href={route.url}
