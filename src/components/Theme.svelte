<script lang="ts">
	import { isDarkMode, currentTheme } from '$lib/store';
		...
		if (browser) {
			...
			darkModeMediaQuery.addEventListener('change', (e) => {
				if (localStorage.theme === 'system') {
					isDarkMode.update((value) => e.matches);
				}
			});
		}
	});
...
<div class="flex gap-1">
	<button
		...
		class="flex items-center justify-center p-2 rounded {$currentTheme === 'light'
			? 'opacity-100'
	...
	<button
		...
		class="flex items-center justify-center p-2 rounded {$currentTheme === 'dark'
			? 'opacity-100'
	...
	<button
		...
		class="flex items-center justify-center p-2 rounded {$currentTheme === 'system'
			? 'opacity-100'
			: 'opacity-50'} hover:opacity-100 {$currentTheme === 'system'
			? $isDarkMode
				? 'bg-zinc-800'
				: 'bg-zinc-200'
			: 'bg-transparent'}"
